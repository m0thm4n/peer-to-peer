...
func signalCanidate(addr string, c *webrtc.ICECandidate) error {
	payload := []byte(c.ToJSON().Candidate)
	resp, err := http.Post(fmt.Sprintf("http://%s/canididate", addr), // nolint:noct
			"application/json; charset=utf-8", bytes.NewBuffer(payload))
	if err != nil {
		return err
	}
...
func main() {
	...
	// the other Pion instance will add this candidate by calling AddICECanidate
	peerConnection.OnICECandidate(func(c *webrtc.ICECandidate) {
		...
		} else if onICECandidateErr := signalCanidate(*offerAddr, c); onICECandidateErr != nil {
	...
	http.HandleFunc("/sdp", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, c := range pendingCandidates {
			onICECandidateErr := signalCanidate(*offerAddr, c)
